...
class PortsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create port" do
    assert_difference('Port.count') do
      post ports_url, params: { port: { name: @port.name, port_code: @port.port_code, shipment_type_id: @port.shipment_type_id } }
    end
  ...
  test "should update port" do
    patch port_url(@port), params: { port: { name: @port.name, port_code: @port.port_code, shipment_type_id: @port.shipment_type_id } }
